...
///
/// This class requires a fully initialized table view cell (most easily configured in Interface Builder and saved as a nib) that contains drawable subviews.
/// The properties of these subviews serve as a reference so that this view can replicate their appearance using more efficient drawing methods rather than
/// relying on the iPhone SDK to manage and draw multiple <tt>UIView</tt>s.
///
...
///
/// Importantly, the properties of the subviews of #tableViewCell should be set to allow for the drawing methods to function properly. Specifically, the frame
/// of the subview should be valid at the very least. In addition to this, for <tt>UILabel</tt>s, specifying the font, line break mode, and alignment will allow
/// the rendered string to be properly formatted. Making use of Interface Builder is recommended as these properties can quickly be configured visually.
///
...
///
/// This initializer sets the value of the #tableViewCell property. When <tt>-drawRect:</tt> is called on this view, it uses #tableViewCell to determine what
/// <tt>UIView</tt> subviews can be composited.
///
